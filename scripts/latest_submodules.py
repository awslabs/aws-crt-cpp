...
def repo_root() -> str:
    """Absolute path to the top-level directory of the current Git repo."""
...
ROOT = repo_root()
THIS_DIR  = Path(__file__).resolve().parent # where this .py lives
GEN_SH    = THIS_DIR / 'gen-crt-min-versions.sh'

def get_submodules(root=ROOT):
    """
    ...
    if not os.path.exists(modules_file):
        sys.exit(f'No .gitmodules found in {root}')
...
def main():
    ...
    for submodule in submodules:
        ...
        os.chdir(os.path.join(ROOT, submodule['path']))

        version_prefix = 'v'
        # aws-crt-java uses FIPS releases of aws-lc
        if name == 'aws-lc' and 'aws-crt-java' in ROOT:
        ...
        if sync_from != sync_to:                      # we really changed it
            ...
            if not args.dry_run:
                ...
                os.chdir(ROOT)
                run('git', 'add', submodule['path'])
        else:
            print(f"{name:<{name_pad}} {sync_from} ✓")

    print('[+] regenerating CRTMinVersions.cmake')
    subprocess.run([str(GEN_SH)], check=True)
