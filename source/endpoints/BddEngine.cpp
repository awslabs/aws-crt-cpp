    ...
    namespace Crt
    {
        namespace Endpoints
        {
                ...
                : m_engine(nullptr, aws_endpoints_bdd_engine_release)
            {
                ...
                {
                    m_engine = ScopedResource<struct aws_endpoints_bdd_engine>(
                        aws_endpoints_bdd_engine_new_from_bytecode(allocator, bytecodeCursor, partitions),
                        aws_endpoints_bdd_engine_release);
            ...
            Optional<ResolutionOutcome> BddEngine::Resolve(const RequestContext &context) const
            {
                ...
                if (aws_endpoints_bdd_engine_resolve(m_engine.get(), context.GetNativeHandle(), &resolved))
                {
