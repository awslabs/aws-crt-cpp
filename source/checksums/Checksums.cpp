...
namespace Aws
{
    namespace Crt
    {
        uint32_t crc_common(const uint8_t *input, size_t length, uint32_t prev, uint32_t (*checksum_fn)(const uint8_t *, int, uint32_t)) {
        ...
        uint32_t crc32(const uint8_t *input, size_t length, uint32_t prev) {
            return crc_common(input, length, prev, aws_checksums_crc32)
        }
