...
namespace Aws
{
    namespace Crt
    {
        DateTime::DateTime() : m_good(true)
        {
            std::chrono::system_clock::time_point time;
            aws_date_time_init_epoch_millis(&m_date_time,
                    static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::milliseconds>
                            (time.time_since_epoch()).count()));
        }

        DateTime::DateTime(const std::chrono::system_clock::time_point& timepointToAssign) : m_good(true)
        {
            aws_date_time_init_epoch_millis(&m_date_time,
                static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::milliseconds>
                        (timepointToAssign.time_since_epoch()).count()));
        }

        DateTime::DateTime(uint64_t millisSinceEpoch) : m_good(true)
        {
            aws_date_time_init_epoch_millis(&m_date_time, millisSinceEpoch);
        }


        DateTime::DateTime(double epoch_millis) : m_good(true)
        {
            aws_date_time_init_epoch_secs(&m_date_time, epoch_millis);
        }

        DateTime::DateTime(const char* timestamp, DateFormat format)
        {
            ByteBuf timeStampBuf = ByteBufFromCString(timestamp);

            m_good = (aws_date_time_init_from_str(&m_date_time, &timeStampBuf, static_cast<aws_date_format>(format))
                    == AWS_ERROR_SUCCESS);
        }
        ...
        DateTime& DateTime::operator=(double secondsSinceEpoch)
        {
        ...
        DateTime& DateTime::operator=(uint64_t millisSinceEpoch)
        {
            aws_date_time_init_epoch_millis(&m_date_time, millisSinceEpoch);
            m_good = true;
            return *this;
        ...
        DateTime& DateTime::operator=(const char* timestamp)
        {
            ByteBuf timeStampBuf = aws_byte_buf_from_c_str(timestamp);

            m_good = aws_date_time_init_from_str(&m_date_time, &timeStampBuf,
                    static_cast<aws_date_format>(DateFormat::AutoDetect)) == AWS_ERROR_SUCCESS;
            return *this;
        }
        ...
        int DateTime::GetLastError()
        {
            return aws_last_error();
        }

        bool DateTime::ToLocalTimeString(DateFormat format, ByteBuf& outputBuf) const
        {
           return (aws_date_time_to_local_time_str(&m_date_time, static_cast<aws_date_format>(format),
                   &outputBuf) == AWS_ERROR_SUCCESS);
        }

        bool DateTime::ToGmtString(DateFormat format, ByteBuf& outputBuf) const
        {
            return (aws_date_time_to_utc_time_str(&m_date_time, static_cast<aws_date_format>(format),
                                                    &outputBuf) == AWS_ERROR_SUCCESS);
        }
        ...
        uint64_t DateTime::Millis() const
        {
        ...
        uint16_t DateTime::GetYear(bool localTime) const
        {
        ...
        uint8_t DateTime::GetDay(bool localTime) const
        {
        ...
        uint8_t DateTime::GetHour(bool localTime) const
        {
            return aws_date_time_hour(&m_date_time, localTime);
        }

        uint8_t DateTime::GetMinute(bool localTime) const
        {
            return aws_date_time_minute(&m_date_time, localTime);
        }

        uint8_t DateTime::GetSecond(bool localTime) const
        {
        ...
        uint64_t DateTime::CurrentTimeMillis()
        {
            return DateTime::Now().Millis();
        }

        uint8_t DateTime::CalculateCurrentHour()
        {
            return DateTime::Now().GetHour(true);
        }

        double DateTime::ComputeCurrentTimestampSecondsMillis()
        {
