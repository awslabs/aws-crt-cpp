...
namespace Aws
{
    namespace Crt
    {
        ...
        namespace Http
        {
            ...
            using OnStreamOutgoingBody = std::function<enum aws_http_outgoing_body_state(const std::shared_ptr<HttpStream>& stream, ByteBuf& buffer)>;
            using OnIncomingHeaders = std::function<void(const std::shared_ptr<HttpStream>& stream, const struct aws_http_header *headersArray, std::size_t headersCount)>;
            using OnIncomingHeadersBlockDone = std::function<void(const std::shared_ptr<HttpStream>& stream, bool hasBody)>;
            using OnIncomingBody = std::function<void(const std::shared_ptr<HttpStream>& stream, const ByteCursor& data, std::size_t& outWindowUpdateSize)>;
            ...
            private:
                ...
                static void s_onIncomingHeaderBlockDone(struct aws_http_stream *stream, bool has_body, void *user_data) noexcept;
                static void s_onIncomingBody(struct aws_http_stream *stream, const struct aws_byte_cursor *data, size_t *out_window_update_size, void *user_data) noexcept;
                static void s_onStreamComplete(struct aws_http_stream *stream, int error_code, void *user_data) noexcept;

            friend class HttpConnection;
            };

            class AWS_CRT_CPP_API HttpConnection final : std::enable_shared_from_this<HttpConnection>
            {
            public:
                ~HttpConnection();
            ...
            {
            public:
                HttpClient(Io::ClientBootstrap *bootstrap, std::size_t initialWindowSize = SIZE_MAX, Allocator *allocator = DefaultAllocator()) noexcept;

                bool NewConnection(const String& hostName, uint16_t port, const Io::SocketOptions &socketOptions,
                                   const Io::TlsConnectionOptions &tlsConnOptions) const noexcept;
                bool NewConnection(const String& hostName, uint16_t port, const Io::SocketOptions &socketOptions) const noexcept;
            ...
            private:
                ...
                static void s_onClientConnectionSetup(struct aws_http_connection *connection, int error_code, void *user_data) noexcept;
                static void s_onClientConnectionShutdown(struct aws_http_connection *connection, int error_code, void *user_data) noexcept;
            };
        }
    }
}
